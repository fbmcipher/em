...
- **CI Verification Loop**: After completing your changes, you MUST verify all CI checks pass before marking a PR as ready for review. Follow this process:
  ...
  - **Running Puppeteer tests locally**: Docker is available in your environment. To run Puppeteer tests, unset `GITHUB_ACTIONS` so the test script automatically starts the browserless Docker container and Vite dev server:
    ```
    GITHUB_ACTIONS="" yarn test:puppeteer
    ```
  - **Snapshot failures**: If Puppeteer tests fail due to snapshot mismatches or missing snapshots, do NOT delete or manually edit snapshot files. Instead, regenerate them by running:
    ```
    GITHUB_ACTIONS="" yarn test:puppeteer --update
    ```
    Then commit the updated snapshot files and push.

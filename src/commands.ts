...
export const handleGestureEnd = ({ sequence, e }: { sequence: Gesture | null; e: GestureResponderEvent }) => {
  ...
  if (!command && chainableCommandInProgressExclusive) {
    ...
    // Prefer exact concatenation (chainedGesture) over coalesced (chainedGestureCollapsed) so that simple
    // concatenation takes precedence when both would match the same gesture sequence.

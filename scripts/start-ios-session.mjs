 ...
 * Bash call returns immediately, fully immune to that host timeout. The agent then drives the live
 * session through the e2e bridge (src/e2e/iOS/attachExistingSession.ts), not the MCP.
 *
 * What it does:
 *   1. starts a BrowserStack Local tunnel (so the app's baked bs-local.com:3000 server.url resolves
 *      to the runner's dev server, whether it serves HTTP or HTTPS),
 *   2. creates the App Automate session (caps mirror .github/skills/browser-control-ios/SKILL.md),
 *   3. writes the session id to /tmp/em-bs-session.txt (the bridge + heartbeat read it) and a
 *      machine-readable status to /tmp/em-ios-bringup.status,
...
const startTunnel = () =>
  new Promise((resolve, reject) => {
    bsLocal.start({ key, localIdentifier, forceLocal: true }, err => (err ? reject(err) : resolve()))
  })
...
const main = async () => {
  ...
  try {
    browser = await remote({
      user,
      key,
      hostname: process.env.BROWSERSTACK_HUB_HOST ?? 'hub-cloud.browserstack.com',
      port: 443,
      protocol: 'https',
      path: '/wd/hub',
      logLevel: 'error',

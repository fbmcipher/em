...
> **`start_session` is slow by nature — be patient, don't thrash.** It routinely takes **~20–30s (and varies)** before returning a session id. This is **BrowserStack provisioning a physical iPhone** — ~17s of device allocation/cleanup *before Appium even starts*, then a ~2s (cached-WDA) handshake. It is **not** app install (negligible; `noReset`), not the IPA, and not HTTPS. If `start_session` aborts with a transport/timeout error ("operation aborted due to timeout"), that's the **MCP client's request timeout firing before BrowserStack finishes** — not a code fault. **Do not retry 5×** (each retry pays the full ~25s and orphans a build). Instead: surface it as an MCP-timeout issue and ask the user to raise the wdio MCP request timeout (it is a Copilot/MCP-client setting, not a capability or a knob in `@wdio/mcp`). One unhurried retry is fine; repeated hammering is not.
